...
- [x] Display the total amount HAFH spends on manager salaries (as `TotalSalary`) and the total
  amount HAFH spends on manager bonuses (as `TotalBonus`). **5 Points**
- [x] Display the `ManagerID`, `MFName`, `MLName`, `MSalary`, and `MBonus`, for all managers with a salary greater than $50,000 and a bonus greater than $1,000. **5 points**
- [x] Display the `BuildingID`, `BNoOfFloors`, and the managers `MFName `and `MLName `for all buildings. **5 points**
- [x] Display the `BuildingID ` and `AptNo `for all apartments leased by the corporate client `WindyCT` **5 points**
- [x] Display the `InsID `and `InsName `for all inspectors that have any inspections scheduled after `1-APR-2020`. Do not display the same information more than once. **5 points**
- [x] Display the `ManagerID`, `MFName`, `MLName`, `MSalary`, and `MBonus `for the manager with the *lowest* total compensation (defined as salary plus bonus) **5 points**
- [x] Provide all SQL queries and the screenshots
